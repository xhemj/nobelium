...
export default function Header({ navBarTitle, fullWidth }) {
  ...
  const resolveFavicon = (fallback) => !fallback && "/favicon.svg";
...
const HeaderName = forwardRef(function HeaderName(
  ...
  return (
    <p
      ref={ref}
      className="header-name ml-1 translate-y-[1px] font-medium text-gray-600 dark:text-gray-300 capture-pointer-events grid-rows-1 grid-cols-1 items-center h-full"
      onClick={onClick}
    >
      {postTitle && (
        <span className="post-title row-start-1 col-start-1 text-base">
          {postTitle}
        </span>
      )}
