...
const components = {
  ...
  image_nobelium: (e) => {
    const src = e?.block?.properties?.source?.[0]?.[0];
    const title = e?.block?.properties?.title?.[0]?.[0];
    const width = e?.block?.format?.block_width;
    const aspectRatio = e?.block?.format?.block_aspect_ratio;
    const height = width ? (width * aspectRatio).toFixed(0) : null;
    const id = e?.block?.id;

    const displaySrc = `https://www.notion.so/image/${encodeURIComponent(
      src
    ...
    return (
      ...
      height &&
      src && (
        <Zoom zoomImg={{ alt: title, width, height }}>
          <Image
            src={displaySrc}
            alt={title}
            width={width}
